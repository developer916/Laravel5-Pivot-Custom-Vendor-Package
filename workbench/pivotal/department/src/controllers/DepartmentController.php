...
class DepartmentController extends DepartmentBaseController {
    ...
    public function view(DepartmentInterface $department) {
        ...
        $data['classes'] = $department->classesForYear();
...
//        $limedata = new LimeData();
        ...
        if ($user->role == \User::DEPARTMENT_HEAD && $user->department_id == $department->id) {
            //dd($data['classes'], $user->classesForYear());
            $teacherClasses = $user->classesForYear();
            $depClasses = $data['classes'];
            $depClasses = $depClasses->merge($teacherClasses);
            $data['classes'] = $depClasses;
        }

        usort($cycles, function($a, $b) {
            $aStartDate = \DateTime::createFromFormat('d/m/Y', $a->start_date)->format('Y-m-d');
            $bStartDate = \DateTime::createFromFormat('d/m/Y', $b->start_date)->format('Y-m-d');
        ...
        return $this->get_view('department::pages.department-view', $data)
    ...
    public function edit(SchoolInterface $school, DepartmentInterface $department = null) {
        ...
        return $this->get_view('department::pages.department-edit', $data);
    }
