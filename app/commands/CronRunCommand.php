...
class CronRunCommand extends Command {
	...
	{
		$this->saturdays(function(){
			\Artisan::call('db:backup');
		});
		
		
		$this->everyMinute(function(){
			$reportCron = new Pivotal\Report\Queue\CronJob();
			$reportCron->run();
		...
		$this->daily(function(){
			...
			\Artisan::call('cycle:report');
		});


		$this->weekdays(function() {
		    // notify teachers of their class survey links, one weekday day before commencement
			$this->messages[] = App::make('Pivotal\Cycle\Controllers\CycleController')->send_notifications();
			// auto email to be sent 3 days before the end of a survey cycle
			$this->messages[] = App::make('Pivotal\Cycle\Controllers\CycleController')->send_pre_notifications();
		});
	...
	protected function everyMinute(callable $callback)
	{
